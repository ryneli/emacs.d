...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  (defun mm-margin-current-window ()
   ...
   (let ((curr-pos (point))
     ...
     (mm-delete-overlays)
     ;; if left-margin less than 2, set it to 2
     (if (or (not (car (window-margins))) (< (car (window-margins)) 2))
         (set-window-margins nil 2 nil))
     (mm-margin-line curr-line (format "%2d" curr-line) nil)
     ;; numbering lines below
     (forward-line)
     (setq line 1)
     (while (and (not (eobp)) (<= (point) endp))
       (mm-margin-line (line-number-at-pos) (format "%2d" line) nil)
       (forward-line)
       (setq line (1+ line)))
     (goto-char curr-pos)
